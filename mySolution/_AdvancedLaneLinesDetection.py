...
def image_saving_warp(img, dir, pts, save=GLOBAL_SAVE, show=GLOBAL_SHOW):
...
def color_and_gradient(img, b_thres=(165, 255), l_thres=(205, 255), s_thres=(175, 195), sx_thres=(40, 120), save=GLOBAL_SAVE, dir=dir_c_b):
    ...
    if save:
        fs = 20
        f003, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(16, 6)); f003.tight_layout()
        ...
        ax3.set_title('Sobel X', fontsize=fs); ax3.imshow(sxbinary, cmap='gray')
        # ax4.set_title('S of HLS', fontsize=fs); ax4.imshow(s_binary, cmap='gray')
        plt.subplots_adjust(left=0., right=1, top=0.9, bottom=0.)
        plt.savefig(dir+str(datetime.datetime.now())+'.jpg', dpi='figure', format='jpg')
...
def detect_lanes(img): # Assuming you have created a warped binary image called "img"
    ...
    offset = (leftx_base + rightx_base - img.shape[1]) / 2.0 * (891 / 640)  # transform warped to real world: 1162-271=891, 960-320=640
    offset_m = offset * LaneLine.xm_per_pix
    lll.line_bases.append(offset_m)
    lll.line_base_pos = rll.line_base_pos = np.average(lll.line_bases)
    ...
    if LOG_LEVEL:
        ...
        print('lll.line_base_pos', lll.line_base_pos)
...
# clip1 = VideoFileClip("../project_video.mp4").subclip(33,34) # good for deviation from center
